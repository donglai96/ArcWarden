...
D.table(["判据", "阈值", "状态"],
        [["G1 线性增长率", "同估计器(带通包络最陡窗)|1−γ_δf/γ_ff|<0.2",
          "PASS(0.855)"],
         ["G2 谱形态", "主导模同带;LB 峰位与线性理论 ω_r 一致",
          "PASS(0.31-0.36 vs 理论 0.319)"],
         ["G3 权重有界", "线性段 rms(w_d) << 1(taud=0)",
          f"PASS({wd_now:.1e},种子量级)"],
         ["G4(后续)", "饱和段 w_d 行为:本次 δf 从低 10^3 种子出发,t=6000 内不达饱和;"
          "若获准接管需补一次长 δf 独跑覆盖饱和段", "预登记,未执行"]],
        [1.30, 3.62, 1.45], size=8.6)
D.text("两臂均已完成。判定方法的一处现场修订(对称适用、已记录):预登记的"
       "W_EM 增长率比较对 full-f 不可测——其总能量的线性段藏在高于 δf 10^6 倍的"
       "散粒噪声底之下,能测到的窗口已被部分饱和压低(扣底噪拟合 3.7e-4/ωpe,"
       "仍系统性偏低)。正式 G1 改为同估计器比较:赤道探针 B1 带通(0.2-0.45 Ωe)"
       "包络的最陡 800/ωpe 滑窗,各臂在自己的有效幅度区间 [3×底噪, 0.6×峰值] 内"
       "拟合。结果:γ_ff=2.02e-3/ωpe,γ_δf=1.73e-3/ωpe,比值 0.855 → G1 通过。"
       "G2:主导模胜出后 δf 谱峰 0.346-0.359,full-f 增长窗峰 0.314-0.354,"
       "线性理论 ω_r=0.319 Ωe(γ_max=7.44e-3 Ωe,k=0.69 ωpe/c)居中,同带一致 "
       "→ 通过。G3:线性段末 rms(w_d)=1.07e-3(种子量级)→ 通过。")
