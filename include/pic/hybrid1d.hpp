...
static __global__ void k_ampere(H1DView v) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= v.NN) return;
    ...
    const double Jy  = (double)v.jhy[i] - v.nc * v.vcy[i];
    const double Jz  = (double)v.jhz[i] - v.nc * v.vcz[i];
...
static __global__ void k_push_deposit(H1DView v) {
    ...
    // deposit J at midpoint with v^{n+1/2}  (J = q n v, q = -1)
    {
        ...
        const float cy = (float)(-v.w0j * vy), cz = (float)(-v.w0j * vz);
...
// out[3] += hot kinetic energy Σ w0 (γ-1)   (w0 = w0j·dx)
static __global__ void k_energy_hot(H1DView v, double* out) {
    __shared__ double sK;
    ...
    if (p < v.np) {
        ...
        k = (v.nonrel ? 0.5 * u2 : sqrt(1.0 + u2) - 1.0) * v.w0j * v.dx;
    }
    atomicAdd(&sK, k);
    __syncthreads();
    if (threadIdx.x == 0) atomicAdd(&out[3], sK);
}
...
public:
    ...
    explicit Hybrid1D(const Chirp1DParams& P) : P_(P) {
        ...
        diag_ = DeviceArray<double>(4); bmax_ = DeviceArray<int>(1);
    ...
    Energies energies() {
        ...
        return {h[0], h[1], h[2], h[3], std::sqrt((double)b2)};
    }
    ...
    void run(bool verbose = true) {
        ...
        for (long n = 0; n < P_.nsteps; ++n) {
            ...
            if (n_energy > 0 && n1 % n_energy == 0) {
                auto e = energies();
                std::fprintf(energy_f_, "%.6f,%.9e,%.9e,%.9e,%.9e,%.6e\n",
                             n1 * P_.dt, e.we, e.wb, e.wc, e.wh, e.bmax);
...
private:
    ...
    void open_outputs() {
        ...
        if (P_.energy_stride > 0) {
            ...
            std::fprintf(energy_f_, "t,WE,WB,Wcold,Whot,bmax\n");
        }
        if (P_.nphase > 0)
