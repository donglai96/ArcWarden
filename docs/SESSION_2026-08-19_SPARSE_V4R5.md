...
## 六、V4R7-DF-A2 的发现:全局模式阈值(2026-08-19)

A=2(γ_max=4.53e-3 Ωe)的 δf 跑在 δB~1e-6 处停滞:赤道探针包络
增长 5.8e-4 → 2.8e-4 → ≈0(t>8000),主导模频率 0.301 Ωe 与理论
0.309 一致,wd 无异常。五探针复查(t 9500–11800):各探针在
+1.8e-4 与 −1.8e-4 之间摇摆,均值 ≈ +0.7e-4/ωpe——**A=2、nh=1%
在本几何处于全局模式阈值边缘**(净增长 ≲1e-4,比 A=2.5 的活跃
探针低一个量级,实际时间内到不了饱和):单程对流增益与逃逸
接近抵消。对照:A=2.5(V4R8)同期多探针 7–9e-4 且持续加速,
明确在阈值上方——阈值位于 A≈2 附近。

意义:(1) 2D 特有的对流损耗阈值,位于 A∈(2,3)、nh=1%、本几何,
1D 模拟原理上给不出;(2) 停滞水平(δB/B~5e-6)远低于 full-f 底噪
(1.3e-3)——只有 δf 能看到该现象;(3) 与 B_th 类阈值物理直接
相关,是 V5(阈值标定)的第一个数据点。

处置:停跑(80k 步,状态已 checkpoint),按 A 阶梯上移至 A=2.5
(γ_max=6.01e-3 Ωe,ω_r=0.314;预计饱和 B_w/B₀~8e-3,元素
Ωe·τ~300)重启 = V4R8-DF-A25。

## 七、V4R8-DF-A25 的发现:外侧列模式选择与有限源折减的陡降

A=2.5 未停滞但增长缓慢(总 By² 能量率 2.7e-4/ωpe 稳定),且波能量
**集中在 L≈4020–4050 的外侧 shell 列**(占 ~55%,L0 列仅 ~10%)——
全局模式选择单程增益最大的列:线长 ∝ L、Ωe ∝ L⁻³(低 3% → 有效
ωpe/Ωe 高)。探针与场线诊断都在 L0=4000 线上,指错了列。

**有限源全局折减实测阶梯**(γ_global/γ_homogeneous,幅度率):
A=3:~45% → A=2.5:~11% → A=2:~2%(阈值边缘)。折减随 A 陡降
——低 A 的共振区随 marginal frequency 收缩,单程增益跌得比 γ_max
快。推论:(1) 观测级低幅度元素在本盒需要更长的源区(更大 L0 或
更长 runway)或更高 nh;(2) 低 A deck 的探针线应放在 L≈4020–4040;
(3) 我的 A 阶梯时长表用 45% 常数折减外推——已被推翻,记录在案。

处置:80k 步停跑(checkpoint 在 80000),切 V4R9-DF-A3(δf,
A=3——XVAL 已证明其在探针线上健康增长),目标:真饱和 + 干净
元素(Ωe·τ~240,A=5 的 3 倍)+ G4 饱和段判定。
