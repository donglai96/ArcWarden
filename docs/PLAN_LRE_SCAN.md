...
## v2(2026-08-09,用户指令:A 太大,按 07-30 基础版重设计)

v1 扫描(Lu-x10 f0:A=5、nh=0.010、hybrid 26.563° 墙)结果:×10
element 跨 0.682 / ×8 FLOOD 0.205 / ×6 element 停 0.409 但 2× over-amp /
×5 FLOOD 0.266。诊断:A=5 的线性驱动带顶到 0.83、γ_max=1.3e-2,弱梯度
臂阈值 ∝lre⁻⁴ 塌掉后宽带噪声全数点火 → flood 吃掉分岔。v1 deck+数据
2026-08-09 已删(用户指令)。
**v2 基底 = decks/giant_x4_atmo40.ini(07-30 "LRE-SCAN x4 ARM 1")**:
该版在最弱梯度 lre=3326 上产出离散元素列车(t2500–18000,0.2→0.45–0.5,
无 flood)。全臂固定:conecut biMax uth=(0.19784,0.24391,0.24391)
(裸 A=0.52,cone-cut 有效 ~0.75–0.88,线性带只到 ~0.44),nh=0.0178,
ppc=28000 统一,**wall MLAT=40°**(Lx=2·g(40°)·lre,g=0.757895),
x=atmo,batm=cone_b=7.4057(墙=loss cone,装载与边界同算子,α_LC=21.6°),
nd=500,numax=0.1,dx=0.26,dt=0.15,dipole profile,seed 20260720,
**t10000**,eline ON。唯一变量 lre。

| 臂 | lre | 压缩 | nx | Lx | markers | 显存 |
|---|---|---|---|---|---|---|
| l1330 | 1330.504 | ×10 | 7757 | 2016.82 | 217M | ~12GB |
| l1663 | 1663.13 | ×8 | 9696 | 2520.96 | 271M | ~15GB |
| l2218 | 2217.68 | ×6 | 12929 | 3361.54 | 362M | ~20GB |
| l2661 | 2661.01 | ×5 | 15514 | 4033.64 | 434M | ~24GB |
| l3326 | 3326.26 | ×4 | 19392 | 5041.92 | 543M | ~30GB |

跑序:×4 先(=基础版复现+eline,锚点),再 ×10(最强梯度、阈值最高,
风险=NO-RISER 无点火,属合法第三态),然后 ×6/×8/×5。判定与 §5–7 判据
不变(冻结 ridge audit + 3-state map + S + P_ω)。v1 记录的教训保留:
P_ω 翻转点跟随各臂端点;stop 臂 |S| 未→1。
